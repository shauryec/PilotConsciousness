  ...
  return (
    <main className="login-page">
      <section className="login-panel password-panel">
        <div className="brand-lockup"><span className="brand-star">✦</span><div><strong>Pilot Consciousness</strong><small>Training Portal</small></div></div>
        <div><p className="eyebrow">Account security</p><h1>Choose your password.</h1><p>Create the password you will use for future portal sign-ins.</p></div>
        <form onSubmit={savePassword}>
          ...
          <button className="button button-light" type="submit" disabled={busy}>{busy ? 'Saving…' : 'Save password'} <span>→</span></button>
          {message && <p className="form-message" role="status">{message}</p>}
        </form>
        <small>Passwords are handled securely by the portal authentication service.</small>
      </section>
    </main>
  )
}
...
function Login() {
  ...
  function changeMode(nextMode: 'login' | 'reset') {
    setMode(nextMode)
    setMessage('')
  }
  ...
  return (
    <main className="login-page">
      <section className="login-panel">
        ...
        {mode === 'login' ? (
          <>
            <div><p className="eyebrow">Secure account access</p><h1>Continue your training.</h1><p>Sign in with the email address and password associated with your course.</p></div>
            <form onSubmit={signIn}>
              ...
              <button className="button button-light" type="submit" disabled={busy}>{busy ? 'Signing in…' : 'Sign in'} <span>→</span></button>
              <button className="login-link" type="button" onClick={() => changeMode('reset')}>Set or forgot your password?</button>
              {message && <p className="form-message" role="status">{message}</p>}
            </form>
          </>
        ) : (
          <>
            <div><p className="eyebrow">Password setup</p><h1>Set your password.</h1><p>We will email one secure link to create or replace your password.</p></div>
            <form onSubmit={sendPasswordReset}>
              ...
              <button className="button button-light" type="submit" disabled={busy}>{busy ? 'Sending…' : 'Send password setup link'} <span>→</span></button>
              <button className="login-link" type="button" onClick={() => changeMode('login')}>Back to sign in</button>
              {message && <p className="form-message" role="status">{message}</p>}
            </form>
          </>
        )}
        <small>Access is limited to invited Pilot Consciousness students and instructors. New accounts cannot register here.</small>
      </section>
    </main>
  )
}
