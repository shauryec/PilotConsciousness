...
function GradeTally({ grades }: { grades: Array<{ grade: OgmuiGrade }> }) {
  const counts = (Object.keys(gradeLabels) as OgmuiGrade[]).map((grade) => ({ grade, count: grades.filter((entry) => entry.grade === grade).length })).filter((entry) => entry.count > 0)
  return <span className="grade-tally" aria-label={counts.map((entry) => `${entry.count} ${gradeLabels[entry.grade]}`).join(', ') || 'No grades'}>{counts.length ? counts.map((entry) => <span key={entry.grade}><Grade value={entry.grade} /><b>{entry.count}</b></span>) : <small>No grades</small>}</span>
}

function GradeSheetSummary({ attempt, lesson, grades }: { attempt: LessonAttempt; lesson?: PortalLesson; grades: Array<{ grade: OgmuiGrade }> }) {
  return <>
    <div className="grade-sheet-identity"><small>{formatDate(attempt.conducted_at)} · Attempt {attempt.attempt_number}</small><strong>Lesson {lesson?.lesson_number} · {lesson?.title ?? 'Untitled lesson'}</strong></div>
    <div className="grade-sheet-time"><span><b>{hoursFromMinutes(attempt.ground_minutes ?? 0).toFixed(1)}</b> ground</span><span><b>{hoursFromMinutes(attempt.flight_minutes ?? attempt.training_minutes ?? 0).toFixed(1)}</b> flight</span><span><b>{hoursFromMinutes(attempt.simulator_minutes ?? 0).toFixed(1)}</b> sim</span></div>
    <GradeTally grades={grades} />
    <span className="grade-sheet-chevron" aria-hidden="true">⌄</span>
  </>
}
...
function LessonsView({ profile, workspace, run }: { profile: PortalProfile; workspace: StaffWorkspace; run: Runner }) {
  const activeEnrollments = workspace.enrollments.filter((entry) => entry.status === 'active')
  const [enrollmentId, setEnrollmentId] = useState(activeEnrollments[0]?.id ?? '')
  ...
  const versionPhases = workspace.phases.filter((phase) => phase.course_version_id === version?.id).sort((a, b) => a.phase_number - b.phase_number)
  const phaseOrder = new Map(versionPhases.map((phase) => [phase.id, phase.phase_number]))
  const phaseIds = versionPhases.map((phase) => phase.id)
  const lessons = workspace.lessons.filter((lesson) => phaseIds.includes(lesson.phase_id)).sort((a, b) => (phaseOrder.get(a.phase_id) ?? 0) - (phaseOrder.get(b.phase_id) ?? 0) || a.lesson_number - b.lesson_number)
  ...
  useEffect(() => {
    if (!enrollmentId && activeEnrollments[0]) setEnrollmentId(activeEnrollments[0].id)
  ...
  }, [activeEnrollments, enrollmentId, lessonId, lessons])
  ...
  return <section className="workspace-view">
    <div className="view-heading"><div><p className="eyebrow dark">Today’s training</p><h1>Lessons</h1><p>Open the lesson with the student, review its objectives and ACS Tasks, then grade and close it after training.</p></div><div className="grade-key">{(Object.keys(gradeLabels) as OgmuiGrade[]).map((grade) => <span key={grade}><Grade value={grade} />{gradeLabels[grade]}</span>)}</div></div>
    <div className="lesson-command-grid">
      <aside className="panel lesson-roster">
        <div className="lesson-roster-head"><div><p className="eyebrow dark">Active students</p><h2>Training roster</h2></div><span>{activeEnrollments.length}</span></div>
        <div className="lesson-roster-list">{activeEnrollments.map((entry) => {
          const student = workspace.profiles.find((candidate) => candidate.id === entry.student_id)
          const activeAttempt = workspace.attempts.find((attempt) => attempt.enrollment_id === entry.id && attempt.status === 'draft')
          const requiredRepeat = workspace.remediations.find((requirement) => requirement.enrollment_id === entry.id && !requirement.resolved_at && requirement.reason === 'U')
          return <button className={entry.id === enrollmentId ? 'active' : ''} type="button" onClick={() => setEnrollmentId(entry.id)} key={entry.id}><ProfileAvatar profile={student ?? { ...profile, id: entry.student_id, full_name: studentName(workspace, entry.student_id) }} className="profile-avatar-directory" /><div><strong>{studentName(workspace, entry.student_id)}</strong><small>{courseName(workspace, entry.course_version_id)}</small></div><b className={activeAttempt ? 'open' : requiredRepeat ? 'repeat' : ''}>{activeAttempt ? 'Open' : requiredRepeat ? 'Repeat' : 'Ready'}</b></button>
        })}</div>
      </aside>
      <div className="lesson-command-main">
        <article className="panel lesson-navigator">
          <div className="lesson-navigator-head"><div><p className="eyebrow dark">{enrollment ? studentName(workspace, enrollment.student_id) : 'No student selected'}</p><h2>{version ? courseName(workspace, version.id) : 'Select an active student'}</h2></div>{openAttempt && <span className="status-pill">Lesson open</span>}</div>
          <div className="lesson-phase-list">{versionPhases.map((phase) => {
            const phaseLessons = lessons.filter((lesson) => lesson.phase_id === phase.id)
            const containsSelection = phaseLessons.some((lesson) => lesson.id === selectedLesson?.id)
            return <details open={containsSelection || expandedPhases.has(phase.id)} onToggle={(event) => { if (containsSelection) return; const isOpen = event.currentTarget.open; setExpandedPhases((current) => { const next = new Set(current); if (isOpen) next.add(phase.id); else next.delete(phase.id); return next }) }} key={`${enrollmentId}-${phase.id}`}><summary><span>Phase {String(phase.phase_number).padStart(2, '0')}</span><strong>{phase.title}</strong><b>{phaseLessons.length} lesson{phaseLessons.length === 1 ? '' : 's'}</b></summary>{phaseLessons.length ? <div>{phaseLessons.map((lesson) => {
              const attempt = workspace.attempts.find((entry) => entry.enrollment_id === enrollmentId && entry.lesson_id === lesson.id && entry.status === 'published')
              const required = repeatRequirement?.required_lesson_id === lesson.id
              const isSelected = lesson.id === selectedLesson?.id
              return <button className={`${isSelected ? 'selected' : ''} ${required ? 'repeat' : ''}`} disabled={Boolean(openAttempt && !isSelected) || Boolean(repeatRequirement && !required)} type="button" onClick={() => setLessonId(lesson.id)} key={lesson.id}><span>{lesson.lesson_number}</span><div><strong>{lesson.title}</strong><small>{required ? 'Repeat required' : attempt ? 'Prior attempt on file' : lesson.kind}</small></div><b>→</b></button>
            })}</div> : <p>No lessons have been added to this phase.</p>}</details>
          })}</div>
        </article>
        <article className="panel grade-editor">
        {activeEnrollments.length ? <>
          {selectedLesson && items.length ? <>
            <div className="grade-editor-head"><div><p className="eyebrow dark">{openAttempt ? `Open since ${formatDateTime(openAttempt.opened_at ?? openAttempt.created_at)}` : repeatRequirement ? 'Repeat required' : 'Lesson briefing'}</p><h2>Lesson {selectedLesson.lesson_number} · {selectedLesson.title}</h2><p>{studentName(workspace, enrollment?.student_id ?? '')} · Attempt {(workspace.attempts.filter((entry) => entry.enrollment_id === enrollmentId && entry.lesson_id === selectedLesson.id).at(0)?.attempt_number ?? 0) + (openAttempt ? 0 : 1)}</p></div><span>{items.length + (openAttempt ? 0 : openRemediations.filter((entry) => !plannedMappings.some((mapping) => mapping.acs_item_id === entry.acs_item_id)).length)} ACS Task{items.length === 1 ? '' : 's'}</span></div>
            ...
            {!openAttempt ? <>
              ...
              <div className="opening-items opening-items-grid">{items.map((item) => <div key={item.id}><AcsTaskHeading item={item} /></div>)}</div>
...
export function StaffPortal({ profile, workspace, refresh }: { profile: PortalProfile; workspace: StaffWorkspace; refresh: () => Promise<void> }) {
  ...
  const content = view === 'overview' ? <StaffOverview profile={profile} workspace={workspace} navigate={setView} /> : view === 'students' ? <StudentsView profile={profile} workspace={workspace} run={run} /> : view === 'courses' ? <CoursesView workspace={workspace} run={run} /> : view === 'lessons' ? <LessonsView profile={profile} workspace={workspace} run={run} /> : view === 'audit' ? <AuditView workspace={workspace} /> : view === 'profile' ? <ProfileView profile={profile} onChanged={refresh} /> : <LibraryView profile={profile} workspace={workspace} run={run} />
...
type StudentView = 'dashboard' | 'course' | 'acs' | 'library' | 'profile'
...
const studentNavItems: Array<{ id: StudentView; label: string; short: string }> = [
  ...
  { id: 'acs', label: 'Grade Sheet', short: 'GS' },
  { id: 'library', label: 'Documents & Videos', short: 'DV' },
  { id: 'profile', label: 'Profile', short: 'ME' },
]

function latestStudentGrade(workspace: StudentWorkspace, acsItemId: string) {
  const closedAttempts = workspace.attempts.filter((attempt) => attempt.status === 'published').sort((a, b) => new Date(b.conducted_at).getTime() - new Date(a.conducted_at).getTime())
...
function studentLessonComplete(workspace: StudentWorkspace, lessonId: string) {
  ...
  const hasClosedAttempt = workspace.attempts.some((attempt) => attempt.lesson_id === lessonId && attempt.status === 'published')
  return hasClosedAttempt && ids.length > 0 && ids.every((id) => ['O', 'G', 'M'].includes(latestStudentGrade(workspace, id)?.grade.grade ?? ''))
}

function studentCourseProgress(workspace: StudentWorkspace) {
  const completedLessons = workspace.lessons.filter((lesson) => studentLessonComplete(workspace, lesson.id)).length
  const phaseFractions = workspace.phases.map((phase) => {
    const lessons = workspace.lessons.filter((lesson) => lesson.phase_id === phase.id)
    return lessons.length ? lessons.filter((lesson) => studentLessonComplete(workspace, lesson.id)).length / lessons.length : 0
  })
  const percent = phaseFractions.length ? Math.round((phaseFractions.reduce((total, value) => total + value, 0) / phaseFractions.length) * 100) : 0
  const completedPhases = phaseFractions.filter((value) => value === 1).length
  return { percent, completedLessons, totalLessons: workspace.lessons.length, completedPhases, totalPhases: workspace.phases.length }
}
...
function StudentDashboard({ profile, workspace, navigate }: { profile: PortalProfile; workspace: StudentWorkspace; navigate: (view: StudentView) => void }) {
  ...
  const progress = studentCourseProgress(workspace)
  ...
  const courseComplete = progress.totalPhases > 0 && progress.percent === 100 && !unresolved.length && !openAttempt

  return <>
    <section className="welcome-row"><div><p className="eyebrow dark">Active course · Revision {workspace.version.revision}</p><h1>Welcome back, {profile.full_name.split(/\s+/)[0]}.</h1><p className="lede">Course progress reflects completed lessons across every phase. Unsatisfactory and Incomplete Tasks remain open until resolved.</p></div><div className="progress-ring" style={{ '--progress': `${progress.percent * 3.6}deg` } as CSSProperties}><div><strong>{progress.percent}%</strong><span>Course</span></div></div></section>
    <section className="dashboard-grid">
      <article className="next-lesson panel panel-dark"><div className="panel-kicker"><span>{openAttempt ? 'Lesson open' : repeat ? 'Repeat required' : nextLesson ? 'Next lesson' : 'Course status'}</span><span>{currentPhase ? `Phase ${String(currentPhase.phase_number).padStart(2, '0')}` : courseComplete ? 'Complete' : 'Awaiting review'}</span></div><p className="lesson-number">{nextLesson ? String(nextLesson.lesson_number).padStart(2, '0') : courseComplete ? '✓' : '—'}</p><h2>{nextLesson?.title ?? (courseComplete ? 'Course requirements complete' : 'Instructor review required')}</h2><p>{openAttempt ? 'Review the objective and Task headings with your instructor before training begins.' : repeat ? 'A prior Unsatisfactory result requires another attempt of this lesson.' : nextLesson?.objective ?? 'Your instructor will review course completion and next steps.'}</p>{nextLesson && <dl className="lesson-meta"><div><dt>Type</dt><dd>{nextLesson.kind}</dd></div><div><dt>Planned time</dt><dd>{nextLesson.planned_ground_minutes + nextLesson.planned_training_minutes} min</dd></div><div><dt>Open items</dt><dd>{unresolved.length}</dd></div></dl>}<button className="button button-light" onClick={() => navigate('course')}>{openAttempt ? 'Review open lesson' : 'Open course'} <span>→</span></button></article>
      <article className="panel recent-debrief"><div className="panel-heading"><div><p className="eyebrow dark">Latest instructor remarks</p><h2>{latestLesson ? `Lesson ${latestLesson.lesson_number}` : 'No published record yet'}</h2></div>{latest && <span className="status-pill">Published</span>}</div>{latest ? <><p className="quote">“{latest.what_worked || 'Open the grade sheet for Task remarks.'}”</p><button className="text-button" onClick={() => navigate('acs')}>Open grade sheet <span>→</span></button></> : <EmptyCollection label="Your first remarks will appear here" detail="Published grade sheets remain available throughout your course." />}</article>
    </section>
    ...
    <section className="lower-grid"><article className="panel phase-panel"><div className="panel-heading"><div><p className="eyebrow dark">Course position</p><h2>{workspace.course.name}</h2></div><strong>{progress.completedLessons} / {progress.totalLessons} lessons</strong></div><div className="course-progress-summary"><span><b>{progress.completedPhases}</b> of {progress.totalPhases} phases complete</span><span><b>{progress.completedLessons}</b> lessons complete</span></div><div className="phase-list">{workspace.phases.map((phase) => { const phaseLessons = workspace.lessons.filter((lesson) => lesson.phase_id === phase.id); const done = phaseLessons.filter((lesson) => studentLessonComplete(workspace, lesson.id)).length; return <div className={phase.id === currentPhase?.id ? 'active' : ''} key={phase.id}><span>{String(phase.phase_number).padStart(2, '0')}</span><strong>{phase.title}</strong><small>{phaseLessons.length ? `${done} of ${phaseLessons.length} lessons` : 'No lessons added yet'}</small></div> })}</div></article><article className="panel attention-panel"><div className="panel-heading"><div><p className="eyebrow dark">Assigned resources</p><h2>Course library</h2></div><button className="quiet-link" onClick={() => navigate('library')}>View all</button></div>{workspace.resources.length ? <div className="attention-list">{workspace.resources.slice(0, 3).map((resource) => <div key={resource.id}><span className="resource-mini">{resource.kind === 'video' ? '▶' : 'DOC'}</span><div><strong>{resource.title}</strong><p>{resource.description || resource.kind}</p></div></div>)}</div> : <EmptyCollection label="No resources assigned" detail="Documents and videos from your instructor will appear here." />}</article></section>
  </>
}
...
function StudentGradeSheet({ workspace }: { workspace: StudentWorkspace }) {
  ...
  const unresolved = workspace.remediations.filter((entry) => !entry.resolved_at)
  return <section className="workspace-view"><div className="view-heading"><div><p className="eyebrow dark">{workspace.publication.code} · Permanent record</p><h1>Grade Sheet</h1><p>Each closed lesson is one permanent grade sheet. Open a row to review its time, OGMUI grades, and instructor remarks.</p></div><div className="grade-key">{(Object.keys(gradeLabels) as OgmuiGrade[]).map((grade) => <span key={grade}><Grade value={grade} />{gradeLabels[grade]}</span>)}</div></div>
    ...
    <div className="record-list">{closedAttempts.length ? closedAttempts.map((attempt) => <StudentGradeSheetCard attempt={attempt} workspace={workspace} key={attempt.id} />) : <EmptyCollection label="No closed grade sheets" detail="The first permanent grade sheet will appear after your instructor closes a lesson." />}</div>
  </section>
}
...
export function StudentPortal({ profile, workspace, refresh }: { profile: PortalProfile; workspace: StudentWorkspace; refresh: () => Promise<void> }) {
  ...
  const content = view === 'dashboard' ? <StudentDashboard profile={profile} workspace={workspace} navigate={setView} /> : view === 'course' ? <StudentCourse workspace={workspace} /> : view === 'acs' ? <StudentGradeSheet workspace={workspace} /> : view === 'profile' ? <ProfileView profile={profile} onChanged={refresh} /> : <StudentLibrary workspace={workspace} />
  const openAttempt = workspace.attempts.find((attempt) => attempt.status === 'draft')
  const repeat = workspace.remediations.find((entry) => !entry.resolved_at && entry.reason === 'U')
  ...
  return <div className="portal-shell"><aside className="sidebar"><Brand /><nav>{studentNavItems.map((item) => <button className={view === item.id ? 'active' : ''} key={item.id} onClick={() => setView(item.id)}><span>{item.short}</span>{item.label}</button>)}</nav><div className="course-chip"><span>{openAttempt ? 'Lesson open' : repeat ? 'Repeat required' : 'Active course'}</span><strong>{workspace.course.short_name}</strong><small>{nextLesson ? `Lesson ${nextLesson.lesson_number} · ${nextLesson.title}` : 'Course requirements complete'}</small></div><div className="account-chip"><ProfileAvatar profile={profile} /><div><strong>{profile.full_name}</strong><small>Student</small></div><button aria-label="Sign out" title="Sign out" onClick={() => void supabase?.auth.signOut()}>↗</button></div></aside><main className="portal-main"><header className="mobile-header"><Brand /><button className="mobile-account" onClick={() => setView('profile')} aria-label="Open profile"><ProfileAvatar profile={profile} /></button></header><nav className="mobile-nav student-mobile-nav">{studentNavItems.map((item) => <button className={view === item.id ? 'active' : ''} key={item.id} onClick={() => setView(item.id)}><span>{item.short}</span>{item.label}</button>)}</nav><div className="main-inner">{content}</div></main></div>
}
