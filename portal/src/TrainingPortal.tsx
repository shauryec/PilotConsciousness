...
function lessonName(workspace: StaffWorkspace, lessonId: string) {
  ...
  return lesson ? `Lesson ${lesson.lesson_number}` : 'Unknown lesson'
}
...
function StudentDesk({ profile, student, workspace, run, back }: { profile: PortalProfile; student: PortalProfile; workspace: StaffWorkspace; run: Runner; back: () => void }) {
  ...
  return <section className="workspace-view student-desk">
    ...
    {tab === 'dashboard' && <>
      <section className="student-desk-top"><article className="panel activation-card"><p className="eyebrow dark">{openAttempt ? 'Lesson open' : 'Next lesson'}</p><h2>{nextLesson ? `Lesson ${nextLesson.lesson_number}` : 'Course review needed'}</h2>{nextLesson && <span className="lesson-activity">{lessonKindLabel(nextLesson.kind)}</span>}<p>{nextLesson?.objective || 'There is no remaining lesson to activate.'}</p><div><button className="button button-primary" disabled={!nextLesson} onClick={() => openDeskTab('lessons')}>{openAttempt ? 'Continue lesson' : 'Activate next lesson'} <span>→</span></button><button className="button button-outline" onClick={() => openDeskTab('lessons')}>Choose another lesson</button></div></article><article className="panel student-metrics"><p className="eyebrow dark">Training position</p><dl><div><dt>Course completion</dt><dd>{completePercent}%</dd></div><div><dt>Lessons complete</dt><dd>{completeLessonIds.size} / {courseLessons.length}</dd></div><div><dt>Lesson completion rate</dt><dd>{attempts.length ? `${Math.round((completeLessonIds.size / attempts.length) * 100)}%` : '—'}</dd></div></dl><small>Projection updates from completed course lessons; it is not an ACS pass percentage.</small></article></section>
      <article className="panel nested-course"><div className="panel-heading"><div><p className="eyebrow dark">Course overview</p><h2>{enrollment ? courseName(workspace, enrollment.course_version_id) : 'No course assigned'}</h2></div><span>{courseLessons.length} lessons</span></div>{coursePhases.map((phase) => { const lessons = courseLessons.filter((lesson) => lesson.phase_id === phase.id); return <details key={phase.id}><summary><span>Phase {String(phase.phase_number).padStart(2, '0')}</span><strong>{phase.title}</strong><b>{lessons.filter((lesson) => completeLessonIds.has(lesson.id)).length} / {lessons.length}</b></summary><ol>{lessons.map((lesson) => <li key={lesson.id}><span className={completeLessonIds.has(lesson.id) ? 'complete-dot' : ''}>{completeLessonIds.has(lesson.id) ? '✓' : lesson.lesson_number}</span><div><strong>Lesson {lesson.lesson_number}</strong><small>{lessonKindLabel(lesson.kind)}</small></div></li>)}</ol></details>})}</article>
    </>}
        ...
        <div className="builder-columns">
          <article className="panel builder-panel">
            ...
            {currentPhase ? <>
              ...
              <div className="lesson-builder-list">{phaseLessons.map((lesson) => <button className={lesson.id === selectedLesson?.id ? 'active' : ''} onClick={() => setLessonId(lesson.id)} key={lesson.id}><span>{lesson.lesson_number}</span><div><strong>Lesson {lesson.lesson_number}</strong><small>{lessonKindLabel(lesson.kind)} · {lesson.planned_ground_minutes + lesson.planned_training_minutes} planned min</small></div><b>→</b></button>)}</div>
          ...
          <article className="panel builder-panel">
            ...
            {selectedLesson ? <>
              <div className="lesson-summary"><span>{lessonKindLabel(selectedLesson.kind)}</span><strong>Lesson {selectedLesson.lesson_number}</strong><p>{selectedLesson.objective}</p><small>{selectedLesson.planned_ground_minutes} ground · {selectedLesson.planned_training_minutes} training min</small></div>
      ...
      <article className="panel grade-editor">
        {workspace.enrollments.length ? <>
          ...
          {selectedLesson && items.length ? <>
            <div className="grade-editor-head"><div><p className="eyebrow dark">{openAttempt ? `Open since ${formatDateTime(openAttempt.opened_at ?? openAttempt.created_at)}` : repeatRequirement ? 'Repeat required' : 'Ready to open'}</p><h2>Lesson {selectedLesson.lesson_number}</h2><p>{lessonKindLabel(selectedLesson.kind)} · {studentName(workspace, enrollment?.student_id ?? '')} · Attempt {(workspace.attempts.filter((entry) => entry.enrollment_id === enrollmentId && entry.lesson_id === selectedLesson.id).at(0)?.attempt_number ?? 0) + (openAttempt ? 0 : 1)}</p></div><span>{items.length + (openAttempt ? 0 : openRemediations.filter((entry) => !plannedMappings.some((mapping) => mapping.acs_item_id === entry.acs_item_id)).length)} ACS Task{items.length === 1 ? '' : 's'}</span></div>
...
function StudentDashboard({ profile, workspace, navigate }: { profile: PortalProfile; workspace: StudentWorkspace; navigate: (view: StudentView) => void }) {
  ...
  useEffect(() => {
    const label = document.querySelector('.phase-panel .panel-heading > strong')
    if (label) label.textContent = `${completedLessons} / ${lessons.length} lessons`
...
export function StudentPortal({ profile, workspace, refresh }: { profile: PortalProfile; workspace: StudentWorkspace; refresh: () => Promise<void> }) {
  ...
  useEffect(() => {
    const label = document.querySelector('.portal-shell .course-chip small')
    if (label) label.textContent = nextLesson ? `Lesson ${nextLesson.lesson_number} · ${lessonKindLabel(nextLesson.kind)}` : 'Course sequence complete'
  }, [nextLesson?.id, nextLesson?.kind, nextLesson?.lesson_number])
